ServerEvents.recipes(e => {
    const minerals = ["copper", "iron", "diamond", "coal", 'emerald', 'gold', 'redstone'];
    const dep = e.recipes.createDeploying;
    const singularities = [];
    minerals.forEach(r => {
        const inter = 'kubejs:incomplete_'+r+'_singularity';
        e.recipes.create.sequenced_assembly([
            Item.of('avaritia:singularity', {Id: 'avaritia:'+r})
        ], 'avaritia:neutron', [
            dep(inter, [inter, 'minecraft:'+r+'_block'])
        ]).transitionalItem(inter).loops(1000);

        singularities.push(Item.of('avaritia:singularity', {Id: 'avaritia:'+r}).strongNBT());
    });
